...
for data in all_graphs:
    hw_type_str = str(data.hw_type)
    if "adder4bit" in hw_type_str:
        data.label = ADDER4BIT
    elif "bcdToseg" in hw_type_str:
        data.label = BCDTOSEG
    elif "encoder8to3" in hw_type_str:
        data.label = ENCODER8TO3
    else:
...
if not model_loaded:
    ...
    # Output layer for 3 classes (adder4bit, bcdToseg, encoder8to3)
    output = nn.Linear(cfg.hidden, 3)
